...
struct scene parser(const char *path)
{
  ...
  scene.light_count = 0;
  struct object *objects = malloc(sizeof (struct object));
  ...
  while (fscanf(svati, "%s", instruction) != EOF)
  {
    ...
    else if (strcmp(instruction, "a_light") == 0)
    {
      ...
      scene.light_count++;
      lights = realloc(lights, sizeof (struct light) * scene.light_count);
    ...
    else if (strcmp(instruction, "object") == 0)
    {
      ...
      scene.object_count++;
      objects = realloc(objects, sizeof (struct object) * scene.object_count);
